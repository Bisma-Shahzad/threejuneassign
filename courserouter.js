...
route.get('/', async (req, res, next) => {
    try {
        ...
        const limit = 5
        const courses = await courseModel.find().limit(limit).skip(page * limit)
        if(!courses){
            res.send(sendResponse(false, null, "no data found")).status(200);
        }else{
            res.send(sendResponse(true, courses)).status(200);
        }
